...
本章是[构建第一个fabric网络](https://hyperledgercn.github.io/hyperledgerDocs/build_network_zh/)的后续，会演示增加一个新组织`Org3`到自动生成的应用通道`mychannel`。它假定你已经对[BYFN](https://github.com/wbwangk/wbwangk.github.io/wiki/Hyperledger#%E5%90%AF%E5%8A%A8%E9%A6%96%E4%B8%AA%E7%BD%91%E7%BB%9Cfirst-network)示范很懂了，包括会使用工具`cryptogen`和`configtxgen`。  
...
```
这是告诉我们`mychannel`的最新配置区块是区块2，**不是**初始区块。默认情况下，`peer channel fetch config`命令返回目标通道的最新配置区块，在本例中是2号区块。当BYFN场景运行时，内嵌脚本执行了两个对通道的附加配置更新。也就是，通过两个通道更新交易定义了两个组织`Org1`和`Org2`的锚peer。象这样，我们有了如下配置序列：区块0，初始区块；区块1，Org1锚peer更新；区块2，Org2锚peer更新。  
...
```
这个新proto`org3_update.pb`包含了Org3定义和指向Org1和Org2材料的高级指针。我们能够放弃大量的Org1和Org2的MSP材料和修改策略信息，因为这些数据已经存在于通道的初始区块中。因此，我们只需要两个配置的增量信息。  
...
```
几乎完成！我们现在的CLI容器中有了一个protobuf二进制文件`org3_update_in_envelope.pb`，然而在可以成功递交这个更新前，我们需要必要的Admin用户签名。我们通道的更新策略(mod_policy)被设置成默认的“MAJORITY”(多数)，这意味着我们需要来自两个初始组织Org1和Org2的管理员签署这个更新。如果我们没有获得这两个签名，则排序服务会因无法满足策略而拒绝这个交易。首先，我们用Org1的Admin签署这个更新proto。记住CLI容器是用Org1 MSP材料引导的，所以我们只需要简单地发送`peer channel signconfigtx`命令：
```bash
...
echo $ORDERER_CA && echo $CHANNEL_NAME
```
现在让我们发送呼叫到排序服务，请求`mychannel`的初始区块。由于我们成功的通道更新，订购服务可以验证附加到此呼叫的签名。如果Org3尚未成功添加到通道配置中，则排序服务会拒绝此请求。  
...
```
注意，我们发送了一个`0`来指定我们要获取的通道账本上的第一个区块（即初始区块）。如果我们简单地发送`peer channel fetch config`命令，则我们会接收到区块5，即定义Org3的更新配置。然而，我们不能在一个下游区块开始账本，反而需要加入区块0。  

发送`peer channel join`命令，并传入初始区块`mychannel.block`：
```bash
...
### 总结
重新配置过程的确涉及很多，但是各个步骤都有一个合理的方法。最终目标是形成以protobuf二进制格式表示的增量交易对象，然后收集必要数量的管理员签名，使得重新配置交易处理完成通道的修改策略。`configtxlator`和`jq`工具，与日益增长`peer channel`命令一起，为我们提供了完成这项任务所需的功能。
